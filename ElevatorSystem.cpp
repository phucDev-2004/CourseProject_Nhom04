...
void ElevatorSystem::processRequests()
{
    ...
    PriorityQueue rejectedReqs(100, true);
    ...
    while (true)
    {
        ...
        bool needToGoUp = elevator.cabinPassengers.hasTargetAbove(elevator.currentFloor) || !upQueue.isEmpty() || (!pendingDown.isEmpty() && pendingDown.top().tangGoi > elevator.currentFloor);

        bool needToGoDown = elevator.cabinPassengers.hasTargetBelow(elevator.currentFloor) || !downQueue.isEmpty() || (!pendingUp.isEmpty() && pendingUp.top().tangGoi < elevator.currentFloor);
        ...
        if (elevator.currentDir == LEN && !needToGoUp)
        {
            ...
            while (!pendingDown.isEmpty())
                downQueue.push(pendingDown.pop());
        ...
        else if (elevator.currentDir == XUONG && !needToGoDown)
        {
            ...
            while (!pendingUp.isEmpty())
                upQueue.push(pendingUp.pop());
            continue; // Quay lai dau vong lap de xu ly tiep o huong moi
        }

        // 5. Di chuyen
        if (!actionTaken)
            cout << "     (Di qua tang " << elevator.currentFloor << ")\n";
        elevator.move();
    }
